...
// WP-Optimize advanced-cache.php (written by version: 3.4.2) (homeurl: https://ecslandingdev.feelme.com/) (abspath: /var/www/html/) (do not change this line, it is used for correctness checks)
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://ecslandingdev.feelme.com/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-ecslandingdev.feelme.com.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-ecslandingdev.feelme.com.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-ecslandingdev.feelme.com.php');
}
